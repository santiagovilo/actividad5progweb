...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0,10,'Reporte de Vehiculos',0,1,'C',true);
        $this->Ln(10);
    }
    ...
    function Content()
    {
        ...
        if ($result->num_rows > 0) {
            ...
            $this->Cell(15,10,'Anio',1,0,'C',true);
            $this->Cell(30,10,'Clasificacion',1,0,'C',true);
            $this->Cell(15,10,'ID rep',1,1,'C',true);

            // Datos de los vehiculos
            $this->SetFont('Arial','',12);
            while($row = $result->fetch_assoc()) {
                $this->Cell(10,10,$row['id_vehiculo'],1,0,'C');
                $this->Cell(50,10,$row['descripcion'],1,0,'C');
                $this->Cell(20,10,$row['marca'],1,0,'C');
                $this->Cell(30,10,$row['modelo'],1,0,'C');
                $this->Cell(25,10,$row['color'],1,0,'C');
                $this->Cell(15,10,$row['anio'],1,0,'C');
                $this->Cell(30,10,$row['clasificacion'],1,0,'C');
                $this->Cell(15,10,$row['id_repuesto'],1,1,'C');
            }
        } else {
            $this->Cell(0,10,'No hay vehiculos',0,1,'C');
        }
